...
var playAudioShim = function ( dom, url, t ) {
	...
	WebIM.Helper.getIEVersion < 9 && audioDom.push(d);
	d.jPlayer({
		ready: function () {
		...
		solution: (WebIM.Helper.getIEVersion != 9 ? "html, flash" : "flash"),
		swfPath: "sdk/jplayer",
		supplied: "mp3",
		ended: function () {
			pause.hide();
...
var uploadShim = function ( fileInputId, type ) {
	...
	return new SWFUpload({ 
		...
		, file_dialog_start_handler: function () {
			if ( WebIM.Helper.getIEVersion && WebIM.Helper.getIEVersion < 10 ) {
		...
		, upload_success_handler: function ( file, response ) {
			//处理上传成功的回调
			try{
				var res = WebIM.Helper.parseUploadResponse(response);
...
var handleOpen = function(conn) {
	...
	if ( !WebIM.Helper.isCanUploadFileAsync && typeof uploadShim === 'function' ) {
...
var regist = function() {
	...
	WebIM.Helper.registerUser(options);
};
//注册页面返回登录页面操作
...
var showEmotionDialog = function() {
	...
	emotionFlag = true;
	// WebIM.Helper.EmotionPicData设置表情的json数组
...
var sendText = function() {
	...
	var msgtext = WebIM.utils.parseLink(WebIM.utils.parseEmotions(encode(msg)));
...
var sendPic = function() {
	...
	// WebIM.Helper.getFileUrl为easemobwebim-sdk获取发送文件对象的方法，fileInputId为 input 标签的id值
	var fileObj = WebIM.Helper.getFileUrl('fileInput');
	if (WebIM.Helper.isCanUploadFileAsync && (fileObj.url == null || fileObj.url == '')) {
		alert("请先选择图片");
		return;
	}
	var filetype = fileObj.filetype || '';
	var filename = fileObj.filename;
	if (!WebIM.Helper.isCanUploadFileAsync || filetype.toLowerCase() in pictype) {
		var opt = {
			type : 'chat',
			...
			onFileUploadComplete : function(data) {
				...
				if ( WebIM.Helper.isCanUploadFileAsync && file && file.files) {
...
var sendAudio = function() {
	...
	var filetype = fileObj.filetype || '';
	var filename = fileObj.filename;
	if (!WebIM.Helper.isCanUploadFileAsync || filetype.toLowerCase() in audtype) {
		var opt = {
			type : "chat",
			fileInputId : 'fileInput',
			filename : flashFilename || filename,
...
var sendFile = function() {
	...
	var fileType = fileObj.filetype || '';
	var filename = fileObj.filename;
	if (!WebIM.Helper.isCanUploadFileAsync || fileType.toLowerCase() in filetype) {
		var opt = {
			type : "chat",
			fileInputId : 'fileInput',
...
var handleAudioMessage = function(message) {
	...
	options.onFileDownloadComplete = function(response, xhr) {
		var objectURL = WebIM.Helper.parseDownloadResponse.call(this, response);
		if (WebIM.Helper.getIEVersion != 9 && window.Audio) {
...
var appendMsg = function(who, contact, message, onlyPrompt) {
	...
	if (typeof message == 'string') {
		localMsg = WebIM.Helper.parseTextMessage(message);
